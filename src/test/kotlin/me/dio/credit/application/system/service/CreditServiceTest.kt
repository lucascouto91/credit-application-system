...
class CreditServiceTest {
    ...
    fun `should create credit `() {
        //given
        val credit: me.dio.credit.application.system.entity.Credit = buildCredit()
        val customerId: Long = 1L

        every { customerService.findById(customerId) } returns credit.customer!!
        every { creditRepository.save(credit) } returns credit
        //when
        val actual: me.dio.credit.application.system.entity.Credit = this.creditService.save(credit)
        //then
    ...
    fun `should not create credit when invalid day first installment`() {
        ...
        val credit: me.dio.credit.application.system.entity.Credit = buildCredit(dayFirstInstallment = invalidDayFirstInstallment)

        every { creditRepository.save(credit) } answers { credit }
        //when
        Assertions.assertThatThrownBy { creditService.save(credit) }
            .isInstanceOf(me.dio.credit.application.system.exception.BusinessException::class.java)
    ...
    fun `should return list of credits for a customer`() {
        //given
        val customerId: Long = 1L
        val expectedCredits: List<me.dio.credit.application.system.entity.Credit> = listOf(buildCredit(), buildCredit(), buildCredit())

        every { creditRepository.findAllByCustomerId(customerId) } returns expectedCredits
        //when
        val actual: List<me.dio.credit.application.system.entity.Credit> = creditService.findAllByCustomerId(customerId)
        //then
        Assertions.assertThat(actual).isNotNull
    ...
    fun `should return credit for a valid customer and credit code`() {
        ...
        val credit: me.dio.credit.application.system.entity.Credit = buildCredit(customer = me.dio.credit.application.system.entity.Customer(
            id = customerId
        )
        )
        ...
        //when
        val actual: me.dio.credit.application.system.entity.Credit = creditService.findByCreditCode(customerId, creditCode)
        //then
        Assertions.assertThat(actual).isNotNull
        Assertions.assertThat(actual).isSameAs(credit)
    ...
    companion object {
        private fun buildCredit(
            ...
            customer: me.dio.credit.application.system.entity.Customer = CustomerServiceTest.buildCustomer()
        ): me.dio.credit.application.system.entity.Credit = me.dio.credit.application.system.entity.Credit(
